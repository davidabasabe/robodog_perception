// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from ros_bt_py_interfaces:msg/CapabilityImplementation.idl
// generated code does not contain a copyright notice
#include "ros_bt_py_interfaces/msg/detail/capability_implementation__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `name`
// Member `version`
// Member `tags_dict`
#include "rosidl_runtime_c/string_functions.h"
// Member `preconditions`
#include "ros_bt_py_interfaces/msg/detail/precondition__functions.h"
// Member `tree`
#include "ros_bt_py_interfaces/msg/detail/tree__functions.h"

bool
ros_bt_py_interfaces__msg__CapabilityImplementation__init(ros_bt_py_interfaces__msg__CapabilityImplementation * msg)
{
  if (!msg) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__init(&msg->name)) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
    return false;
  }
  // version
  if (!rosidl_runtime_c__String__init(&msg->version)) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
    return false;
  }
  // preconditions
  if (!ros_bt_py_interfaces__msg__Precondition__Sequence__init(&msg->preconditions, 0)) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
    return false;
  }
  // tags_dict
  if (!rosidl_runtime_c__String__init(&msg->tags_dict)) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
    return false;
  }
  // tree
  if (!ros_bt_py_interfaces__msg__Tree__init(&msg->tree)) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
    return false;
  }
  return true;
}

void
ros_bt_py_interfaces__msg__CapabilityImplementation__fini(ros_bt_py_interfaces__msg__CapabilityImplementation * msg)
{
  if (!msg) {
    return;
  }
  // name
  rosidl_runtime_c__String__fini(&msg->name);
  // version
  rosidl_runtime_c__String__fini(&msg->version);
  // preconditions
  ros_bt_py_interfaces__msg__Precondition__Sequence__fini(&msg->preconditions);
  // tags_dict
  rosidl_runtime_c__String__fini(&msg->tags_dict);
  // tree
  ros_bt_py_interfaces__msg__Tree__fini(&msg->tree);
}

bool
ros_bt_py_interfaces__msg__CapabilityImplementation__are_equal(const ros_bt_py_interfaces__msg__CapabilityImplementation * lhs, const ros_bt_py_interfaces__msg__CapabilityImplementation * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->name), &(rhs->name)))
  {
    return false;
  }
  // version
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->version), &(rhs->version)))
  {
    return false;
  }
  // preconditions
  if (!ros_bt_py_interfaces__msg__Precondition__Sequence__are_equal(
      &(lhs->preconditions), &(rhs->preconditions)))
  {
    return false;
  }
  // tags_dict
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->tags_dict), &(rhs->tags_dict)))
  {
    return false;
  }
  // tree
  if (!ros_bt_py_interfaces__msg__Tree__are_equal(
      &(lhs->tree), &(rhs->tree)))
  {
    return false;
  }
  return true;
}

bool
ros_bt_py_interfaces__msg__CapabilityImplementation__copy(
  const ros_bt_py_interfaces__msg__CapabilityImplementation * input,
  ros_bt_py_interfaces__msg__CapabilityImplementation * output)
{
  if (!input || !output) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__copy(
      &(input->name), &(output->name)))
  {
    return false;
  }
  // version
  if (!rosidl_runtime_c__String__copy(
      &(input->version), &(output->version)))
  {
    return false;
  }
  // preconditions
  if (!ros_bt_py_interfaces__msg__Precondition__Sequence__copy(
      &(input->preconditions), &(output->preconditions)))
  {
    return false;
  }
  // tags_dict
  if (!rosidl_runtime_c__String__copy(
      &(input->tags_dict), &(output->tags_dict)))
  {
    return false;
  }
  // tree
  if (!ros_bt_py_interfaces__msg__Tree__copy(
      &(input->tree), &(output->tree)))
  {
    return false;
  }
  return true;
}

ros_bt_py_interfaces__msg__CapabilityImplementation *
ros_bt_py_interfaces__msg__CapabilityImplementation__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros_bt_py_interfaces__msg__CapabilityImplementation * msg = (ros_bt_py_interfaces__msg__CapabilityImplementation *)allocator.allocate(sizeof(ros_bt_py_interfaces__msg__CapabilityImplementation), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ros_bt_py_interfaces__msg__CapabilityImplementation));
  bool success = ros_bt_py_interfaces__msg__CapabilityImplementation__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ros_bt_py_interfaces__msg__CapabilityImplementation__destroy(ros_bt_py_interfaces__msg__CapabilityImplementation * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__init(ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros_bt_py_interfaces__msg__CapabilityImplementation * data = NULL;

  if (size) {
    data = (ros_bt_py_interfaces__msg__CapabilityImplementation *)allocator.zero_allocate(size, sizeof(ros_bt_py_interfaces__msg__CapabilityImplementation), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ros_bt_py_interfaces__msg__CapabilityImplementation__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ros_bt_py_interfaces__msg__CapabilityImplementation__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__fini(ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ros_bt_py_interfaces__msg__CapabilityImplementation__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence *
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * array = (ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence *)allocator.allocate(sizeof(ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__destroy(ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__are_equal(const ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * lhs, const ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ros_bt_py_interfaces__msg__CapabilityImplementation__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence__copy(
  const ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * input,
  ros_bt_py_interfaces__msg__CapabilityImplementation__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ros_bt_py_interfaces__msg__CapabilityImplementation);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ros_bt_py_interfaces__msg__CapabilityImplementation * data =
      (ros_bt_py_interfaces__msg__CapabilityImplementation *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros_bt_py_interfaces__msg__CapabilityImplementation__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ros_bt_py_interfaces__msg__CapabilityImplementation__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ros_bt_py_interfaces__msg__CapabilityImplementation__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
